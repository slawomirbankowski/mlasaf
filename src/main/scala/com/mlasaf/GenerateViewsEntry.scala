...
object GenerateViewsEntry {
  ...
  def main(args : Array[String]) = {
    ...
    implicit val connmssql = java.sql.DriverManager.getConnection("jdbc:sqlserver://localhost\\SQLEXPRESS2014;DatabaseName=mlasaf08", "sa", "sapass");
    ...
    cols.map(x => x.TABLE_NAME).distinct.sorted.foreach( tableName => {
      ...
      val tableAliasName = " _" + tableName
      ...
      val fkTablesFieldSeq =  joinFkTableColumnList.foreach(fkTable => {
        ...
        cols.filter(fkt => fkt.TABLE_NAME.equals(fkTable._1)).foreach(fktab => {
          fkTablesFieldList.append(" ,  _" + fktab.TABLE_NAME + "." + fktab.COLUMN_NAME + " as " + fktab.TABLE_NAME + "_" + fktab.COLUMN_NAME);
        })
      });

      val colListDefinition = fieldWithAliasList + fkTablesFieldList.toString();
      val joinDefinition = joinFkTableColumnList.map(c => "\n   join " + c._1 + " _" + c._1 + " on " + tableAliasName + "." + c._2 + " = _" +c._1 + "." + c._2 ).mkString(" ")
