...
import java.util.Date
 ...
 def insertDownloadTransformColumnDto(dto : DownloadTransformColumnDto): DownloadTransformColumnDto = { 
    ...
    if (rs.next()) { 
      ...
      SQL("select * from downloadTransformColumn where downloadTransformColumnId = {pkValue} ").on("pkValue" -> pkValue).as(anorm.Macro.namedParser[DownloadTransformColumnDto].single); 
    } else { 
      null; 
    } 
 } 
