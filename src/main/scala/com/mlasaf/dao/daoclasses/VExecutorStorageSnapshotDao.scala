...
import java.util.Date
 ...
 class VExecutorStorageSnapshotDao extends DaoBase { 
  ...
  def getVExecutorStorageSnapshotsByField(fieldName : String, fieldValue : String) : List[VExecutorStorageSnapshotDto] = {
   implicit val connection = getConnection();
   val dtos : List[VExecutorStorageSnapshotDto]= SQL("select * from vExecutorStorageSnapshot where " + fieldName + " = {fieldValue} ").on("fieldValue" -> fieldValue).as(anorm.Macro.namedParser[VExecutorStorageSnapshotDto].*);
   releaseConnection(connection);
   dtos
  }
