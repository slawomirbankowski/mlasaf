...
object DaoTest {
  ...
  def main(args : Array[String]) = {
    ...
    println("All PK columns: " + pkCols.size);
    println("All PK columns: " + fkCols.size);
    ...
    cols.map(x => x.TABLE_NAME).distinct.sorted.foreach( t => {
      ...
      val caseClassDef = "case class "+ t.charAt(0).toUpper + t.substring(1) + "Dto ( " + cols.filter(c => c.TABLE_NAME.equals(t)).map(col => "val " + col.COLUMN_NAME + " : " + col.SCALA_TYPE).mkString(", ") + " ) extends BaseDto { fields = \"" + fieldList + "\"; pkFields = \"" + pkFieldList + "\"; fkFields = \"" + fkFieldList + "\" } ";
